...
def _extract_orbax_values(d, path=""):
    ...
    if isinstance(d, dict):
        return {
            k: _extract_orbax_values(v, f"{path}.{k}" if path else k)
            for k, v in d.items()
        }
    return d
        ...
        if path in ["stage1", "stage2", "stage3"]:
            ...
            for k, v in d.items():
                if k.isdigit():
                    ...
                    raise ValueError(
                        f"Unexpected non-numeric key '{k}' in {path} state"
                    )
            return State(state_dict)

        state_dict = {
            k: _dict_to_state(v, f"{path}.{k}" if path else k) for k, v in d.items()
        }
        return State(state_dict)
...
class CIFARTrainMCDropoutPreActResNet18(luigi.Task):
    ...
    def run(self) -> None:
        seed_everything(self.seed)
        Path(self.output().path).parent.mkdir(parents=True, exist_ok=True)
        x_train_full, y_train_full, x_te, y_te, n_cls = _load_cifar_dataset(
            self.dataset
        )
        ...
        t0 = time.time()
        model = MCDropoutPreActResNet18(
            n_classes=n_cls, dropout_rate=self.dropout_rate, rngs=nnx.Rngs(self.seed)
        )
    ...
    def run(self) -> None:
        ...
        for _ in range(self.epochs - warmup_ep):
            ...
            swag_sq_mean = jax.tree.map(
                lambda m, p: (m * n_swag + p**2) / n, swag_sq_mean, cur
            )
            n_swag += 1
        swag_var = jax.tree.map(
            lambda sq, m: jnp.maximum(sq - m**2, 1e-8), swag_sq_mean, swag_mean
        )
        ...
        with open(self.output().path, "wb") as f:
            pickle.dump(
                {"state": state, "swag_mean": swag_mean, "swag_var": swag_var}, f
            )
        print(f"Checkpoint saved to {self.output().path}")
...
class CIFARPJSVD(luigi.Task):
    ...
    def run(self) -> None:
        ...
        t0 = time.time()
        print(
            f"  Finding {self.n_directions} directions via randomized SVD "
...
class CIFARMLPJSVDv(luigi.Task):
    ...
    def run(self) -> None:
        ...
        t0 = time.time()
        print(
            f"  Finding {self.n_directions} multi-layer directions via randomized SVD..."
        )
        v_opts, sigmas = find_pjsvd_directions_randomized_svd(
            model_fn_flat,
            W_joint_flat,
        ...
        for p_size in self.perturbation_sizes:
            ...
            print(f"  Precompute time: {time.time() - t1:.2f}s")
            m = _evaluate_cifar(
                f"ML-PJSVD BN Refit (scale={p_size})",
                ens,
                x_te,
                y_te,
                n_cls,
...
class CIFARPnC(luigi.Task):
    ...
    def output(self) -> luigi.LocalTarget:
        ...
        return luigi.LocalTarget(
            str(
                Path("results")
                / self.dataset
                / f"pnc{calib_str}_s{self.target_stage_idx}b{self.target_block_idx}_{self.safe_subspace_backend}_k{self.n_directions}_n{self.n_perturbations}"
    ...
    def run(self) -> None:
        ...
        pre_act_chunks, T_orig_chunks = compute_cifar_block_preacts(
            ...
            X_sub,
            self.chunk_size,
            self.target_stage_idx,
            self.target_block_idx,
            w1_orig,
        )
    ...
    def output(self) -> luigi.LocalTarget:
        ...
        return luigi.LocalTarget(
            str(
                Path("results")
                / self.dataset
                / f"pnc_allblocks{calib_str}_{self.safe_subspace_backend}_k{self.n_directions}_n{self.n_perturbations}"
                f"_ps{ps}_e{self.epochs}_subsetsize{self.subset_size}_seed{self.seed}.json"
            )
        )
