...
class UCIBaseTask(luigi.Task):
    ...
    def _setup_task(
        ...
        x_train_full, y_train_full, x_test, y_test = load_uci(
...
class UCITrainBaseModel(UCIBaseTask):
    ...
    def run(self) -> None:
        act_fn, x_tr, y_tr, x_va, y_va, _, _, in_dim, out_dim, hidden_dims = (
            self._setup_task()
        )
...
class UCIStandardEnsemble(UCIBaseTask):
    ...
    def run(self) -> None:
        act_fn, x_tr, y_tr, x_va, y_va, x_test, y_test, in_dim, out_dim, hidden_dims = (
            self._setup_task()
        )
...
class UCIMCDropout(UCIBaseTask):
    ...
    def run(self) -> None:
        ...
        t0 = time.time()
        model = MCDropoutRegressionModel(
            in_dim,
            out_dim,
            nnx.Rngs(self.seed),
            hidden_dims=hidden_dims,
...
class UCIPJSVD(UCIBaseTask):
    ...
    def output(self) -> luigi.LocalTarget:
        ...
        return luigi.LocalTarget(
            str(
                ...
                / "uci"
                / self.dataset
                / f"pjsvd_{self.safe_subspace_backend}_k{self.n_directions}_n{self.n_perturbations}_ps{ps}_act-{self.activation}_seed{self.seed}.json"
            )
        )
    ...
    def run(self) -> None:
        ...
        for p_size in self.perturbation_sizes:
            ...
            m = _evaluate_uci(
                f"PJSVD-{self.activation} (size={p_size})",
                ens,
                x_test,
                y_test,
...
class UCIMultiLayerPJSVD(UCIBaseTask):
    ...
    def output(self) -> luigi.LocalTarget:
        ...
        return luigi.LocalTarget(
            str(
                ...
                / "uci"
                / self.dataset
                / f"ml_pjsvd_{self.safe_subspace_backend}_k{self.n_directions}_n{self.n_perturbations}_ps{ps}_act-{self.activation}_seed{self.seed}.json"
            )
        )
    ...
    def run(self) -> None:
        ...
        for p_size in self.perturbation_sizes:
            ...
            m = _evaluate_uci(
                f"ML-PJSVD-{self.activation} (size={p_size})",
                ens,
                x_test,
                y_test,
                sidecar_path=f"{base_npz}_ps{p_size}.npz",
...
class UCISubspaceInference(UCIBaseTask):
    ...
    def run(self) -> None:
        ...
        T = (
            ...
            else 2.0 * float(np.sqrt(len(x_tr)))
        )
