...
from pnc_core.util import _split_data, _predictive_mean_var        # noqa: E402
from pnc_core.metrics import compute_ood_metrics, compute_nll      # noqa: E402
...
def _row(env, seed, factor, value, num, anchor_val, cfg, m, status="ok", err=""):
    ...
    row.update(environment=env, seed=seed, factor=factor, factor_value=value,
               ...
               status=status, error_message=err, git_commit=GIT,
               # derived from the config, never a literal: the previous hardcoded
               # "zero" contradicted ANCHOR["toward_orig"]=True and mislabelled the
               # entire stored sweep.
               ridge_center=("original" if cfg.get("toward_orig") else "zero"),
