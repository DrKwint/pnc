...
class _Bottleneck(nnx.Module):
    ...
    def __init__(
        self, in_channels: int, planes: int, strides: int = 1, rngs: nnx.Rngs = None
    ):
        out_channels = planes * self.expansion
        self.conv1 = _BNConvFixed(
            in_channels, planes, kernel_size=1, strides=1, rngs=rngs
        )
        self.conv2 = _BNConvFixed(
            planes, planes, kernel_size=3, strides=strides, rngs=rngs
        )
        self.conv3 = _BNConvFixed(
            planes, out_channels, kernel_size=1, strides=1, rngs=rngs
        )

        self.downsample = (
            _BNConvFixed(
                in_channels, out_channels, kernel_size=1, strides=strides, rngs=rngs
            )
    ...
    def __call__(
        self, x: Float[Array, _SHAPE_IN], use_running_average: bool = False
...
def _make_resnet_stage(
    in_channels: int, planes: int, n_blocks: int, strides: int, rngs: nnx.Rngs
):
...
def _make_preact_resnet_stage(
    in_channels: int, out_channels: int, n_blocks: int, strides: int, rngs: nnx.Rngs
):
    blocks = []
    for i in range(n_blocks):
        s = strides if i == 0 else 1
...
class PreActResNet18(nnx.Module):
    ...
    def __init__(self, n_classes: int = 10, rngs: nnx.Rngs = None):
        ...
        self.stage1 = _make_preact_resnet_stage(
            64, 64, n_blocks=2, strides=1, rngs=rngs
        )
        self.stage2 = _make_preact_resnet_stage(
            64, 128, n_blocks=2, strides=2, rngs=rngs
        )
        self.stage3 = _make_preact_resnet_stage(
            128, 256, n_blocks=2, strides=2, rngs=rngs
        )
        self.stage4 = _make_preact_resnet_stage(
            256, 512, n_blocks=2, strides=2, rngs=rngs
        )
    ...
    def __call__(
        self, x: Float[Array, "batch H W C"], use_running_average: bool = False
    ) -> Float[Array, "batch n_classes"]:
        x = self.stem(x)
        x = self._run_stages(x, use_running_average=use_running_average)
