...
from training import (
    ...
    train_swag_classification_model,
)
...
class MNISTTrainBaseModel(luigi.Task):
    ...
    def run(self) -> None:
        ...
        model = ClassificationModel(
            in_dim, n_cls, nnx.Rngs(self.seed), activation=act_fn
        )
        model = train_classification_model(
            model, x_tr, y_tr, x_va, y_va, steps=self.steps
        )
    ...
    def run(self) -> None:
        ...
        for i in range(self.n_baseline):
            m = ClassificationModel(
                in_dim, n_cls, nnx.Rngs(self.seed + i), activation=act_fn
            )
...
class MNISTMCDropout(luigi.Task):
    ...
    def run(self) -> None:
        ...
        t0 = time.time()
        model = MCDropoutClassificationModel(
            in_dim, n_cls, nnx.Rngs(self.seed), activation=act_fn
        )
        model = train_classification_model(
            model, x_tr, y_tr, x_va, y_va, steps=self.steps
...
class MNISTLaplace(luigi.Task):
    ...
    def run(self) -> None:
        ...
        for prior in self.laplace_priors:
            ...
            m = _evaluate_mnist(
                f"Laplace (prior={prior})", lap_ens, x_test, y_test, n_cls
            )
...
class MNISTPJSVD(luigi.Task):
    ...
    def requires(self) -> luigi.Task:
        return MNISTTrainBaseModel(
            steps=self.steps, seed=self.seed, activation=self.activation
        )
    ...
    def run(self) -> None:
        ...
        t_dir = time.time()
        v_opts_orig, sigmas_orig = _find_pjsvd_directions(
            model_fn_l1, W1, self.n_directions, use_full_span=False, seed=self.seed
        )
        ...
        t_dir = time.time()
        v_opts_full, sigmas_full = _find_pjsvd_directions(
            model_fn_l1, W1, self.n_directions, use_full_span=True, seed=self.seed
        )
        ...
        for p_size in self.perturbation_sizes:
            ...
            m_orig = _evaluate_mnist(
                f"PJSVD Orig (size={p_size})",
                ens_orig,
                x_test,
                y_test,
                n_cls,
            ...
            m_full = _evaluate_mnist(
                f"PJSVD Full Span Least Squares (size={p_size})",
                ens_full,
                x_test,
                y_test,
                n_cls,
...
class MNISTEnsemblePJSVD(luigi.Task):
    ...
    def run(self) -> None:
        ...
        for m_idx in range(self.n_base_models):
            ...
            base_m = ClassificationModel(
                in_dim, n_cls, nnx.Rngs(self.seed + m_idx), activation=act_fn
            )
            base_m = train_classification_model(
                base_m, x_tr, y_tr, x_va, y_va, steps=self.steps
            )
            ...
            print(
                f"  [Member {m_idx + 1}] Finding {self.n_directions} ORIGINAL null-space directions..."
            )
            v_opts_orig, sigmas_orig = _find_pjsvd_directions(
                model_fn_l1,
                W1,
                self.n_directions,
                use_full_span=False,
                seed=self.seed + m_idx,
            )
            print(
                f"  [Member {m_idx + 1}] Finding {self.n_directions} FULL SPAN null-space directions..."
            )
            v_opts_full, sigmas_full = _find_pjsvd_directions(
                model_fn_l1,
                W1,
        ...
        for p_size in self.perturbation_sizes:
            ...
            m_orig = _evaluate_mnist(
                f"Ensemble+PJSVD Orig (size={p_size})",
                hybrid_orig,
                x_test,
                y_test,
                n_cls,
            ...
            m_full = _evaluate_mnist(
                f"Ensemble+PJSVD Full Span Least Squares (size={p_size})",
                hybrid_full,
                x_test,
                y_test,
                n_cls,
...
class MNISTMultiLayerPJSVD(luigi.Task):
    ...
    def run(self) -> None:
        ...
        print(
            f"Finding {self.n_directions} ORIGINAL multi-layer null-space directions..."
        )
        t_dir = time.time()  # renamed from t0
        v_opts_buf_orig = np.zeros(
            (self.n_directions, W1.size + W2.size), dtype=np.float32
        )
        ...
        for k in range(self.n_directions):
            ...
            v_opts_buf_orig[k] = np.array(
                jnp.concatenate([v.flatten() for v in v_opts_list])
            )
        ...
        print(
            f"Finding {self.n_directions} FULL SPAN multi-layer null-space directions..."
        )
        t_dir = time.time()
        v_opts_buf_full = np.zeros(
            (self.n_directions, W1.size + W2.size), dtype=np.float32
        )
        ...
        for p_size in self.perturbation_sizes:
            ...
            m_full = _evaluate_mnist(
                f"ML-PJSVD Full Span Least Squares (size={p_size})",
                ens_full,
                x_test,
                y_test,
                n_cls,
...
class AllMNISTExperiments(luigi.WrapperTask):
    ...
    n_baseline = luigi.IntParameter(default=5)
    n_directions = luigi.IntParameter(default=40)
    perturbation_sizes = luigi.ListParameter(
        default=[20.0, 40.0, 60.0, 80.0, 160.0, 320.0]
    )
    laplace_priors = luigi.ListParameter(default=[1.0, 10.0, 100.0])
    seed = luigi.IntParameter(default=0)
    activation = luigi.Parameter(default="relu")
    ...
    def requires(self) -> list[luigi.Task]:
        ...
        return [
            ...
            MNISTEnsemblePJSVD(
                ...
                n_perturbations=self.n_perturbations,
                perturbation_sizes=self.perturbation_sizes,
                n_base_models=self.n_baseline,
                **shared,
            ),
            MNISTMultiLayerPJSVD(
                n_directions=self.n_directions,
                n_perturbations=self.n_perturbations,
