...
from pnc_core.models import TransitionModel
...
def _ls_or_ridge_solve(
    h_aug: jax.Array,
    target: jax.Array,
    lambda_reg: float,
    w_prior: jax.Array | None = None,
) -> jax.Array:
    """Solve h_aug @ W_aug ≈ target.

    λ=0: pseudoinverse via jnp.linalg.lstsq (handles rank-deficient h_aug with a
    minimum-norm solution; exactly the legacy behavior). w_prior is ignored.
    λ>0, w_prior is None: ridge toward zero (legacy Tikhonov) —
        min ||h_aug W − target||² + λ ||W||²
        W = (h_augᵀ h_aug + λI)⁻¹ h_augᵀ target
    λ>0, w_prior is not None: ridge toward the supplied prior weights —
        min ||h_aug W − target||² + λ ||W − w_prior||²
        W = (h_augᵀ h_aug + λI)⁻¹ (h_augᵀ target + λ w_prior)
    w_prior must match h_aug.shape[1] × target.shape[1]. Regularizes all rows
    uniformly (including the bias row) just like the zero-prior variant.
    """
    if lambda_reg > 0.0:
        D = h_aug.shape[1]
...
class PJSVDEnsemble:
    ...
    def __init__(
        ...
        # CANONICAL DEFAULT (revision closure): the P&C correction objective is
        #     min ||Y_v Theta'^T - Y Theta^T||_F^2 + lambda ||Theta' - Theta||_F^2
        # i.e. the ridge shrinks toward the ORIGINAL affine map, not toward zero.
        # False reproduces historical zero-centered runs only; it is not the
        # algorithm reported in the paper.
        # When True and lambda_reg > 0, the ridge penalty shrinks the correction
        # weights toward the original next-layer weights instead of toward zero.
        # Objective becomes  min ||hW − target||² + λ ||W − W_orig||².
        ridge_toward_orig: bool = True,
        ...
        **kwargs
        ...
        self.ridge_toward_orig = bool(ridge_toward_orig)
    ...
    def _precompute_least_squares(self, dp_all):
        ...
        for i in range(len(self.z_coeffs)):
            ...
            w_prior_aug = None
            if self.ridge_toward_orig and self.lambda_reg > 0.0:
                nw = self.correction_params.get("next_w")
                nb = self.correction_params.get("next_b")
                if nw is not None:
                    if nb is None:
                        nb = jnp.zeros(nw.shape[-1], dtype=nw.dtype)
                    w_prior_aug = jnp.concatenate([nw, nb[None, :]], axis=0)
            W_aug = _ls_or_ridge_solve(h_new_aug, Z_target, self.lambda_reg, w_prior_aug)
    ...
    def _precompute_sequential_ls(self, dp_all):
        ...
        # 1. Compute original pre-activations at each perturbed layer. Computed via
        #    a full-model walk so it is valid for perturbed layers that are not a
        #    contiguous chain from the input (e.g. a single non-first layer). This
        #    value is retained for compatibility; the correction targets below use
        #    ``all_orig_h`` (the full-model activations), not ``orig_preacts``.
        orig_preacts = []
        if hasattr(self.base_model, "layers"):
            _acts = [self.X_sub]
            _h = self.X_sub
            for _idx in range(len(self.base_model.layers)):
                _h = self.activation(
                    _h @ self.base_model.layers[_idx].kernel.get_value()
                    + self.base_model.layers[_idx].bias.get_value())
                _acts.append(_h)
            for l_name in self.layers:
                _li = int(l_name.replace("l", "")) - 1
                orig_preacts.append(
                    _acts[_li] @ self.layer_params[l_name]["W"] + self.layer_params[l_name]["b"])
        ...
        if self.layer_specs is not None:
            ...
            for i in range(n_members):
                ...
                for j, spec in enumerate(self.layer_specs):
                    # z_coeffs is stacked to K_max across layers; truncate to this
                    # layer's actual K (needed when layers differ in K, e.g.
                    # pjsvd_family="random_full" where K_li = D_li).
                    K_li = int(np.asarray(spec["sigmas"]).shape[0])
                    z_row = self.z_coeffs[i, j, :K_li]  # (K_li,)
                    ...
                    if spec.get("identity_basis"):
                        # v_opts = I_{D_li}; skip the matmul to avoid a
                        # D_li × D_li identity allocation (6.4 GB for 40k).
                        dp = coeffs
                    else:
        ...
        for i in range(n_members):
            ...
            for j in range(n_layers):
                ...
                if self.correction_mode == "none":
                    ...
                    w_prior_aug = None
                    if (
                        self.ridge_toward_orig
                        and self.lambda_reg > 0.0
                        and corr_idx < len(base_model.layers)
                    ):
                        w_prior_aug = jnp.concatenate(
                            [W_corr_orig, b_corr_orig[None, :]], axis=0
                        )
                    W_aug = _ls_or_ridge_solve(h_aug, target, self.lambda_reg, w_prior_aug)
                    W_corr = W_aug[:-1, :]
                    b_corr = W_aug[-1, :]
    ...
    def _forward_member(self, x, i, dp_all):
        # Sequential correction for per-layer-independent mode (layer_specs set).
        # Gate must match _precompute_corrections (line ~289) which enters the
        # sequential precompute whenever layer_specs is not None, regardless of
        # layer count. Otherwise dp_all is None here and indexing fails.
        if self.layer_specs is not None and self.correction_mode in ("least_squares", "none"):
    ...
    def _predict_member_intermediate_and_corrected(self, x, i, dp_all):
        # Sequential correction for per-layer-independent mode (layer_specs set).
        # See _forward_member for why the gate must be layer_specs, not n_layers.
        if self.layer_specs is not None and self.correction_mode in ("least_squares", "none"):
...
class SWAGEnsemble:
    ...
    def predict_intermediate(self, x: jax.Array, layer_idx: int = 1) -> jax.Array:
        from pnc_core.util import get_intermediate_state
        hs = []
        for _ in range(self.n_models):
            sampled_m = self._sample_model()
...
# ==============================================================================
from pnc_core.pnc import flatten_conv_kernel_to_patches, solve_chunked_conv2_correction
