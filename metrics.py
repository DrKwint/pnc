...
def compute_nll(mean: jax.Array, variance: jax.Array, targets: jax.Array) -> jax.Array:
    ...
    nll = 0.5 * (
        jnp.log(variance) + (targets - mean) ** 2 / variance + jnp.log(2 * jnp.pi)
    )
    return jnp.mean(nll)


def compute_calibration(
    mean: jax.Array, variance: jax.Array, targets: jax.Array, n_bins: int = 10
) -> jax.Array:
    """
...
def print_metrics(name: str, rmse: float, var: float, nll: float, calib_err: float):
    print(
        f"[{name}] RMSE: {rmse:.5f} | Var: {var:.5f} | NLL: {nll:.5f} | CalibErr: {calib_err:.5f}"
    )


def compute_ood_metrics(
    var_id_scores: np.ndarray, var_ood_scores: np.ndarray
) -> tuple[float, float]:
    """
    Calculates AUROC and AUPR for Out-of-Distribution detection based on uncertainty scores.
    """
    labels = np.concatenate(
        [np.zeros(len(var_id_scores)), np.ones(len(var_ood_scores))]
    )
