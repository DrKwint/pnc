    ...
    for i, entry in enumerate(entries):
        ...
        if split in ("id", "both"):
            if "pred_var_id" in data and "sq_error_id" in data:
                splits_to_plot.append(
                    ("ID", data["pred_var_id"], data["sq_error_id"], color, "o", "-")
                )
        ...
        for split_name, pred_var, sq_err, col, marker, ls in splits_to_plot:
            ...
            if bins is not None:
                ...
                ax.scatter(pv[idx], se[idx], c=[col], alpha=0.25, s=6, marker=marker)
                ax.scatter(
                    [], [], c=[col], label=lbl, marker=marker, s=20
                )  # legend proxy
...
def main():
    ...
    parser.add_argument("--log", action="store_true", help="Use log-log axes")
    parser.add_argument(
        "--bins",
        type=int,
        default=30,
    ...
    parser.add_argument(
        "--seed", type=int, default=0, help="NumPy random seed for scatter subsampling"
    )
