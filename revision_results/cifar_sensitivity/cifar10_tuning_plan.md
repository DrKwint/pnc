...
for SEED in 0 1 2; do
  python -m luigi --module pnc_core.cifar_tasks CIFARPreActSWAG \
...
for SEED in 0 1 2; do
  for PRIOR_SCALE in 0.5 1.0 3.0; do
    python -m luigi --module pnc_core.cifar_tasks CIFARPreActEpinet \
...
```bash
python -m luigi --module pnc_core.cifar_tasks CIFARMultiBlockPnC \
...
```bash
for SEED in 0 1 2; do
  python -m luigi --module pnc_core.cifar_tasks CIFARPnC \
    --dataset cifar10 --epochs 300 \
...
### Final Report
```bash
python -m pnc_core.report --results_dir results --env cifar10 --fmt md
```
