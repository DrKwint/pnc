...
Usage:
    ...
    python -m pnc_core.json_to_tex_table --out reports/tables/gym_tables.tex
    python -m pnc_core.json_to_tex_table --out reports/tables/gym_tables.txt
...
PROFILES = {
    "gym": {
        ...
        "caption": "{env}. All sweepable hyperparameters (MC Dropout dropout rate, SWAG scale, Subspace temperature, Laplace prior, and PnC perturbation size) selected per (env, seed) by best validation NLL (fallback: ID NLL for runs without nll_val).",
    },
    "cifar": {
        "metrics": [
...
REQUIRE_KEYS = {
    ...
    "family",   # low | random | random_full | all | none
    ...
    "sws",  # SWAG scale (only meaningful for stems that emit _sws or are nested under sws config)
}
...
def _stem_satisfies_clause(stem: str, key: str, value: str) -> bool:
    ...
    if key == "family":
        ...
        if value == "random_full":
            return "_random_full_" in stem
        if value == "random":
            # Match the low-rank random family, not random_full.
            return "_random_" in stem and "_random_full_" not in stem
        if value in ("low", "all"):
    ...
    if key == "sws":
        if value == "none":
            return "_sws" not in stem and "_swss" not in stem
        return bool(re.search(rf"_sws{re.escape(value)}(?![\d.eE+-])", stem))
    return False
...
def fmt_cell(values: list[float], include_std: bool, mode: str = "mean_std", decimals: int = 3) -> str:
    if mode == "median_iqr":
        ...
        return rf"{m:.{decimals}f} (IQR {sp:.{decimals}f})"
...
def fmt_cell_text(values: list[float], include_std: bool, mode: str = "mean_std", decimals: int = 3) -> str:
    if mode == "median_iqr":
        m, sp = median_iqr(values)
        if math.isnan(m):
            return "--"
        if sp is None or not include_std:
            return f"{m:.{decimals}f}"
        return f"{m:.{decimals}f} (IQR {sp:.{decimals}f})"
    mu, sd = mean_std(values)
    if math.isnan(mu):
        return "--"
    if sd is None or not include_std:
...
def _trim_values(values: list[float], ratio: float, higher_better: bool) -> tuple[list[float], list[float]]:
    """Drop per-seed values more than ``ratio`` × the seed-median for that cell.

    Returns (kept, dropped). Higher-is-better metrics are skipped (AUROC/AUPR
    bounded in [0, 1]). Cells with non-positive median are skipped (NLL is
    frequently negative — "10× a negative" is not well-defined).
    """
    if ratio <= 0 or len(values) < 3 or higher_better:
        return list(values), []
    sorted_vals = sorted(values)
    n = len(sorted_vals)
    mid = n // 2
    ...
    kept, dropped = [], []
    for v in values:
        (dropped if v > threshold else kept).append(v)
    return kept, dropped


def trim_seed_outliers_in_rows(
    rows: list[tuple[str, dict[str, list[float]]]],
    metrics,
    ratio: float,
) -> tuple[list[tuple[str, dict[str, list[float]]]], list[tuple[str, str, float, float]]]:
    """Per (row × metric) cell, drop seeds whose value exceeds ratio × cell-median.

    Only applied to lower-is-better metrics with positive seed-median. Returns
    the trimmed rows and a list of (label, metric_key, dropped_value, median)
    tuples for each seed that was removed.
    """
    if ratio <= 0:
        return rows, []
    trimmed_rows: list[tuple[str, dict[str, list[float]]]] = []
    trim_info: list[tuple[str, str, float, float]] = []
    metric_keys = {m for m, _ in metrics}
    for label, row_metrics in rows:
        new_metrics: dict[str, list[float]] = {}
        for key, values in row_metrics.items():
            if key not in metric_keys:
                new_metrics[key] = values
                continue
            kept, dropped = _trim_values(values, ratio, is_higher_better(key))
            if dropped:
                sorted_vals = sorted(values)
                n = len(sorted_vals)
                mid = n // 2
                med = sorted_vals[mid] if n % 2 == 1 else 0.5 * (sorted_vals[mid - 1] + sorted_vals[mid])
                for v in dropped:
                    trim_info.append((label, key, float(v), float(med)))
            new_metrics[key] = kept
        trimmed_rows.append((label, new_metrics))
    return trimmed_rows, trim_info
...
def gym_friendly_name(stem: str) -> str | None:
    ...
    if stem.startswith("pjsvd"):
        ...
        # `_full` appears in two unrelated places: (a) use_full_span=True emits a
        # `_full` marker after the backend string (e.g. ..._projected_residual_full_prob_);
        # (b) the random_full pjsvd family emits `_random_full_` before the backend.
        # Detect (a) by requiring the marker to follow the backend token.
        use_full_span = (
            "_projected_residual_full" in stem
            or "_activation_covariance_full" in stem
        )
        full = " (Full)" if use_full_span else ""
        k = extract_stem_value(stem, "k")
        n = extract_stem_value(stem, "n")
        family = ""
        if "_random_full_" in stem:
            family = "Random-Full"
        elif "_low" in stem:
            family = "Low"
...
def normalize_gym_stem(stem: str, max_over: set[str]) -> str:
    ...
    if "family" in max_over:
        # Strip the longer token first so `_random_full` doesn't collapse into
        # a stray `_full` that then gets re-interpreted as the use_full_span flag.
        normalized = normalized.replace("_random_full", "")
    ...
    if "sws" in max_over:
        # Strip both single-scale (_sws<v>) and multi-scale-batch (_swss<a>-<b>...)
        # tokens. Strip the longer alias first to avoid leaving a stray prefix.
        normalized = re.sub(r"_swss[0-9.eE+\-]+", "", normalized)
        normalized = re.sub(r"_sws[0-9.eE+\-]+", "", normalized)
    if "act" in max_over:
...
def config_label(stem: str, method: str, profile: str, config: str) -> str:
    if profile == "gym":
        ...
        if stem.startswith("swag") or method.startswith("SWAG"):
            return f"{method} (scale={config})"
        if stem.startswith("subspace") or method.startswith("Subspace"):
            return f"{method} (T={config})"
...
def render_env_table(
    ...
    for row_idx, (label, row_metrics) in enumerate(rows):
        cells = [
            maybe_bold_tex(
                fmt_cell(row_metrics.get(metric, []), include_std, mode=stat_mode, decimals=decimals),
    ...
    if dropped_info:
        note = "Disqualified (mean > threshold $\\times$ col median): " + "; ".join(
            f"{lbl} [{m}={v:.{decimals}f} vs med {med:.{decimals}f}]"
...
def render_env_table_text(
    ...
    for row_idx, (label, row_metrics) in enumerate(rows):
        body.append(
            ...
            + [
                maybe_bold_text(
                    fmt_cell_text(row_metrics.get(metric, []), include_std, mode=stat_mode, decimals=decimals),
                    row_idx in winners.get(metric, set()),
                )
                for metric, _ in metric_specs
            ]
    ...
    if dropped_info:
        ...
        for label, metric, mu, median in dropped_info:
            lines.append(f"  - {label} | {metric}={mu:.{decimals}f} vs median={median:.{decimals}f}")
    return "\n".join(lines)
...
def build_tables(
    ...
    for env_dir in env_dirs:
        ...
        trim_info: list[tuple[str, str, float, float]] = []
        if seed_trim_ratio > 0:
            rows, trim_info = trim_seed_outliers_in_rows(
                rows, profile_cfg["metrics"], seed_trim_ratio
            )
        dropped_info: list[tuple[str, str, float, float]] = []
        ...
        if trim_info:
            for lbl, key, v, med in trim_info:
                dropped_info.append(
                    (f"{lbl} [seed-trim]", key, v, med)
                )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        metavar="K",
        help=(
            "Per (row × metric) cell, drop individual seeds whose value exceeds K × "
            "the cell-median before computing mean/std. Applies only to lower-is-better "
            "metrics with positive median. Useful for heavy-tailed seed distributions "
            "(e.g. MC Dropout Far NLL blowups). Default 0 disables; 10 is a reasonable "
            "starting point. Dropped seeds are listed in the caption/footnote."
        ),
    )
    parser.add_argument(
        "--decimals",
        ...
        default=3,
        metavar="N",
        help="Number of digits after the decimal point for all numeric cells. Default: 3.",
    )
    parser.add_argument(
        "--disqualify-ratio",
        type=float,
    ...
    tex = build_tables(
        ...
        seed_trim_ratio=args.seed_trim,
    )
    if args.out:
