...
def _friendly_name(canonical: str) -> str | None:
    ...
    if canonical.startswith("subspace_inference"):
        return f"Subspace (n={_extract(canonical, 'n')}){act_suffix}"
...
def _load_env_results(env_dir: Path) -> dict:
    ...
    """
    groups: dict[str, dict] = defaultdict(lambda: defaultdict(lambda: defaultdict(list)))
...
def _build_section(title: str, env_dir: Path, metrics_cfg: list, fmt: str) -> str:
    ...
    for can, group in nested_items:
        ...
        for config_val, agg in group.items():
            ...
            suffix  = f" (n={n_seeds} seed{'s' if n_seeds != 1 else ''})" if n_seeds > 1 else ""
...
def main():
    parser = argparse.ArgumentParser(description="Print a compact report of experiment results.")
    parser.add_argument("--results_dir", default="results",
                        help="Root results directory (default: results/)")
    parser.add_argument("--env", default=None,
                        help="Filter to a single environment name")
    parser.add_argument("--fmt", choices=["text", "md"], default="text",
    ...
    if args.fmt == "md":
        ...
        parts.append("*Cells show mean ± stddev across seeds (when >1 seed available).*\n")
    else:
        parts.append("\n" + "#" * 72)
        parts.append("  EXPERIMENT RESULTS REPORT")
    ...
    if mnist_dir.exists() and (args.env is None or args.env.upper() == "MNIST"):
        section = _build_section("MNIST Classification", mnist_dir, CLF_METRICS, args.fmt)
        if section.strip():
